@extends('layouts.app')
@section('title', 'Home - BPS Provinsi Sulawesi Selatan')
...
<section class="hero-dashboard">
    ...
    <div class="hero-mark"><img src="{{ asset('assets/img/bps.jpg') }}" alt="Logo BPS"><span>STATISTIK<br>TERPERCAYA</span></div>
</section>
...
<section class="section-heading publications-heading"><div><span class="eyebrow">DATABASE WEBSITE</span><h2>Publikasi Terbaru</h2></div><a class="section-link" href="{{ route('publikasi.index') }}">Lihat semua →</a></section>
<section class="publication-grid">
@forelse($recentPublications as $pub)
<a class="publication-card" href="{{ $pub->link ?: route('publikasi.index') }}" @if($pub->link) target="_blank" rel="noopener" @endif>
    <div class="publication-cover">@if($pub->sampul)<img src="{{ asset('storage/'.$pub->sampul) }}" alt="{{ $pub->judul }}">@else<div class="no-cover">BPS</div>@endif</div>
    <div class="publication-info"><span>{{ optional($pub->tanggal_rilis)->format('d/m/Y') }}</span><h3>{{ $pub->judul }}</h3><small>Lihat publikasi ↗</small></div>
</a>
@empty<div class="empty-publications">Belum ada publikasi pada database.</div>@endforelse
</section>
</main>
<footer class="home-footer"><strong>BPS PROVINSI SULAWESI SELATAN</strong><span>Portal Statistik • {{ date('Y') }}</span></footer>
@endsection

@push('scripts')
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
const API={bps:@json(route('api.bps')),weather:@json(route('api.weather')),earthquake:@json(route('api.earthquake')),infographics:@json(route('api.infographics')),publications:@json(route('api.publikasi'))};
...
async function searchPublication(){const q=document.getElementById('homeSearch').value.trim();const h=document.getElementById('homeSearchHint');if(!q){h.textContent='';return;}try{const r=await fetch(API.publications+'?keyword='+encodeURIComponent(q));const j=await r.json();const items=j.data||[];h.innerHTML=items.length?items.slice(0,5).map(x=>`<a href="${@json(route('publikasi.index'))}?q=${encodeURIComponent(x.judul)}">${escapeHtml(x.judul)}</a>`).join(''):'Tidak ada publikasi yang cocok.';}catch(e){h.textContent='Pencarian sementara tidak tersedia.';}}
